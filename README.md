...
20 configurações em 5 abas. Cada uma tem padrão `default` (padrão), que deixa a escolha do jogo intacta. Um perfil com tudo em padrão não faz nada.
...
| Taxa de Quadros | `[framerate]` | 4 | limite, cadência, método, ritmo |
...
O volt-gui executa `volt-probe` sob o perfil que você está editando. Para cada backend que alcança ele abre uma janela de 1px que nunca é mapeada e cria superfície e swapchain; cria um sampler, registra o que o dispositivo reportou e sai. Nada aparece na tela.

```
volt --probe meuperfil -- volt-probe
```

Sai com 0 em sucesso, 1 onde a instância ou o dispositivo não pôde ser criado, e 2 onde o dispositivo lista o portability subset e falta o suporte de instância de que essa extensão depende. A camada registra fatos do dispositivo na enumeração, então os cartões com dispositivo preenchem de qualquer jeito.

Abre X11 e Wayland, cada um via biblioteca carregada em runtime, então uma máquina sem um reporta o outro e uma máquina sem ambos deixa essas configurações em `padrão`. Isso só afeta modos de apresentação, contagens de imagem e modos alfa. O arquivo carrega uma seção por backend que abriu, e o cartão oferece a união com cada valor nomeando os backends que reportaram: `mailbox (xcb, wayland)`, `immediate (xcb)`. A etiqueta é só rótulo, perfis guardam o valor. Jogos podem abrir gamescope ou superfície Flatpak, e onde a superfície do jogo recusa um valor a camada lida em tempo de execução: contagem de imagens é limitada contra a superfície real, e um modo de apresentação ou alfa rejeitado mantém o valor do jogo com um aviso.
...
**VSync / Modo de Apresentação** `immediate` desligado, `mailbox` vsync de baixa latência, `fifo` vsync clássico, `fifo_relaxed` rasga abaixo da atualização. Apenas esses quatro são oferecidos ou forçados; os que você descartou são escondidos do menu de vsync do jogo, e um modo que uma extensão define é deixado onde o driver o colocou.
...
Um sampler não normalizado, subamostrado, de processamento de imagem ou convertido mantém cada campo que sua própria forma restringe, então uma configuração nomeando um desses campos o deixa em paz e registra uma linha.

Três campos de amostrador, três configurações. `nearest` e `linear` são base sem consulta por trás, então `linear` só é forçado onde o próprio filtro de ampliação, filtro de redução ou modo mipmap do sampler já usa linear; caso contrário essa configuração deixa o sampler em paz e registra uma linha. Retrô é `nearest`/`nearest`/`nearest`. Bilinear é `linear`/`linear`/`nearest`. Trilinear é `linear`/`linear`/`linear`.

**Filtragem Anisotrópica** `desligado` até o que sua GPU reporta. O volt nunca habilita `samplerAnisotropy`; onde o jogo deixou desligado a configuração é ignorada e registrada. Quase todo jogo habilita.

**Viés LOD** desloca a seleção de mipmap para mais nítido ou mais borrado.

**Piso Mip / Teto Mip** níveis de mip mais baixo e mais alto que amostradores podem usar. Um limite forçado que cruzaria o valor do outro campo é descartado em vez de trocado, e registra uma linha.
...
**Alfa para Cobertura** desliga, seja o que o jogo pediu. O volt nunca força ligado: ligado precisa que o fragment shader escreva alfa na location 0, e o volt nunca lê um shader. Só faz algo onde o jogo já renderiza em MSAA.
...
A maioria dos limitadores oferece um limite e um método. O volt oferece quatro configurações. Nada mais no Linux cobre todas as quatro.

**Limite de Quadros** limite no momento da apresentação. Prazos seguem uma linha do tempo fixa em vez da última apresentação, então jitter do agendador não te arrasta abaixo da taxa pedida. Um quadro que perde seu prazo por mais de um intervalo é liberado de imediato e a linha do tempo se reancora a partir dali, em vez de esperar o restante do intervalo já perdido. Mantido por swapchain.
...
| Tela | `vkGetPhysicalDeviceSurfacePresentModesKHR`, `...SurfaceCapabilities(2)KHR`, `vkCreateSwapchainKHR`, `vkCreateSharedSwapchainsKHR`, `vkQueuePresentKHR` |
| Texturas | `vkCreateSampler`, `vkWriteSamplerDescriptorsEXT`, `vkCreateGraphicsPipelines`, `vkCreateComputePipelines`, `vkCreateShadersEXT`, `vkCreateRayTracingPipelines(KHR/NV)`, `vkGetPipelineIndirectMemoryRequirementsNV`, `vkGetPipelineKeyKHR`, `vkCreatePipelineBinariesKHR` |
| Renderização | `vkCreateGraphicsPipelines`, `vkGetPipelineKeyKHR`, `vkCreatePipelineBinariesKHR`, `vkCmdSetAlphaToCoverageEnableEXT`, `vkCmdSetAlphaToOneEnableEXT`, `vkCmdSetDepthClampEnableEXT` |
...
Cada configuração é interceptada em cada caminho que a alcança. Variantes `2`/`EXT`, grupos de dispositivos, swapchains compartilhadas, escritas de amostrador inline e cobertura alfa dinâmica recebem o mesmo tratamento. Uma lista de modos de apresentação que o driver preenche é filtrada no local; uma lista que o jogo fornece na criação da swapchain é reconstruída como cópia própria do volt contendo o modo forçado, e o modo nomeado no present é reconstruído para coincidir.
...
Existe um flake não oficial [volt-gui-nix](https://github.com/keygenesis/volt-gui-nix). Também não mantenho, mas o empacotamento estava bem feito quando foi oferecido aqui, então não vou desencorajar.

Leia o `flake.nix` primeiro. Não por causa do empacotador, mas porque o mesmo vale para qualquer empacotamento que você não escreveu ou que não vem do repositório da sua distro.
...
O comando de inicialização para o perfil selecionado é mostrado no topo da janela, pronto para copiar.

Nomes de perfil devem ser ASCII gráfico não vazio sem espaço, sem separador de caminho, sem `..` e sem byte nulo. Qualquer outro cai no padrão com um aviso. O lançador escreve um perfil comentado no primeiro uso.
...
[volt] present_mode: pedido fifo, aplicado mailbox
[volt] image_count: pedido 3, aplicado 3
[volt] mag_filter: pedido linear, aplicado nearest
[volt] anisotropy: o aplicativo não habilitou samplerAnisotropy
[volt] depth_clamp: o aplicativo não habilitou depthClamp
[volt] frame_limit: aplicado 60
[volt] frame_pacing: o perfil não definiu
```

Cada linha de configuração nomeia primeiro a configuração, depois ou `pedido A, aplicado B` ou o motivo pelo qual a configuração não foi aplicada. O valor aplicado é o que o volt escreveu, então uma configuração que o dispositivo limitou mostra o que caiu em vez do que o perfil diz.

As quatro configurações de Taxa de Quadros não têm valor pedido, já que um jogo nunca diz ao Vulkan qual taxa quer. Elas reportam o que o volt aplicou, ou dizem que o perfil não as definiu.

A linha da GPU mostra `pedido N, aplicado M` quando o perfil define gpu: N é o dispositivo que o jogo usou, M o que seu perfil escolheu. Sem gpu no perfil mostra só `pedido N`.

Cada par distinto de pedido e aplicado imprime uma vez por dispositivo por mais samplers, pipelines ou swapchains que o jogo crie, e uma linha de motivo imprime cada vez que um valor é mantido.
...
| `~/.config/volt-gui/probe.toml` | o que a última sondagem leu, uma seção por backend |
...
O lançador detecta `flatpak run` e roteia pelo wrapper dentro do sandbox. Ele também monta `~/.config/volt-gui` no sandbox, somente leitura, ou leitura-escrita sob `--probe`:
...
Chame o wrapper você mesmo, útil onde só a extensão está instalada. Monte os perfis você também. O wrapper roda dentro do sandbox e não pode fazer isso de lá:

```
flatpak run --filesystem=xdg-config/volt-gui:ro --command=/usr/lib/extensions/vulkan/volt/bin/volt-flatpak com.example.Jogo
VOLT_CONFIG_NAME=meuperfil flatpak run --filesystem=xdg-config/volt-gui:ro --command=/usr/lib/extensions/vulkan/volt/bin/volt-flatpak com.example.Jogo
```

A opção de inicialização Steam não tem espaço para essa flag, então conceda uma vez:

```
flatpak override --user --filesystem=xdg-config/volt-gui:ro com.example.Jogo
```

Então a opção de inicialização é só o wrapper:

```
/usr/lib/extensions/vulkan/volt/bin/volt-flatpak %command%
```

Sem a concessão a camada ainda carrega, só não encontra perfil e deixa toda configuração em paz. Ela registra o caminho que não conseguiu ler.
