    ...
    return {
        ...
        "Configurações": {
            ...
            "Os Três Cartões de Filtro": (
                ...
                ("text", "Um sampler não normalizado, subamostrado, de processamento de imagem ou convertido mantém cada campo que sua própria forma restringe, então uma configuração nomeando um desses campos o deixa em paz e registra uma linha."),
            ),
            "O Limitador de Quadros": (
                ("text", "Limite de Quadros limita a taxa no momento da apresentação. Cadência define em qual taxa o limitador mira, Método define quando ele espera, Ritmo define como, e nenhum dos três faz nada até que Limite esteja definido."),
        ...
        "Uso": {
            ...
            "Vendo o Que Foi Aplicado": (
                ("text", "Execute o jogo por um terminal com VOLT_LOG=info e a camada imprime o que aplicou e o que a superfície ou o dispositivo recusou."),
                ("code", "VOLT_LOG=info volt -- ./jogo", ""),
                ("text", "Cada configuração ganha uma linha: ou o valor que o jogo pediu e o valor que o volt escreveu em seu lugar, ou o motivo pelo qual a configuração não foi aplicada. O valor aplicado é o que o volt escreveu, então uma configuração que o dispositivo limitou mostra o que foi efetivado em vez do que você escolheu."),
                ("text", "Uma configuração que precisa de um recurso de dispositivo que o jogo deixou desmarcado nomeia esse recurso. As configurações de Taxa de Quadros não têm valor pedido, já que um jogo nunca diz ao Vulkan qual taxa quer, então elas reportam o que o volt aplicou ou dizem que o perfil não as definiu."),
                ("text", "A linha da GPU mostra `pedido N, aplicado M` quando você define uma gpu: N é o dispositivo que o jogo usou, M o que seu perfil escolheu. Sem gpu no perfil mostra só `pedido N`."),
                ("text", "Cada par distinto de pedido e aplicado imprime uma vez por dispositivo por mais samplers, pipelines ou swapchains que o jogo crie, e uma linha de motivo imprime cada vez que um valor é mantido."),
            ),
            "A Sondagem": (
