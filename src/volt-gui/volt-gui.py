...
NEW_PROFILE_LABEL: Final[str] = "Novo"
SUBMENU_TITLE: Final[str] = "Aplicar Perfil   "
DELETE_PROFILE_LABEL: Final[str] = "Excluir"
SELECTOR_MIN_WIDTH: Final[int] = 120
SELECTOR_STRETCH: Final[int] = 1
DEFAULT_PROFILE_LABEL: Final[str] = "Padrão"
...
def process_new_profile_save(main_window: QMainWindow) -> None:
    ...
    match (accepted, profile_name is not None and call_new_profile_name_valid(profile_name)):
        case (True, True):
            ...
            process_launch_line_update(main_window)
            process_tray_menu_update(main_window)
            process_notification_display(main_window, "Perfil '" + profile_name.strip() + "' criado.")
            return None
        case (True, False):
            process_notification_display(main_window, "Nome de perfil inválido ou já existe.")
            return None
        case _:
            return None
...
def process_current_profile_delete(main_window: QMainWindow) -> None:
    match main_window.current_profile == DEFAULT_PROFILE:
        case True:
            process_notification_display(main_window, "Não é possível excluir o perfil padrão.")
            return None
        case False:
            match process_yes_no_dialog(main_window, "Excluir Perfil", "Excluir perfil '" + main_window.current_profile + "'?"):
                ...
                case True:
                    ...
                    process_launch_line_update(main_window)
                    process_tray_menu_update(main_window)
                    process_notification_display(main_window, "Perfil excluído.")
                    return None
        ...
        case (False, True):
            match process_yes_no_dialog(main_window, "Aplicar Predefinição", "Aplicar '" + selected_text + "' em '" + main_window.current_profile + "'? Todos os valores serão substituídos."):
                case True:
                    ...
                    process_profile_save(main_window.all_widgets, main_window.current_profile)
                    process_notification_display(main_window, "Predefinição '" + selected_text + "' aplicada ao perfil '" + main_window.current_profile + "'.")
...
def process_tray_menu_update(main_window: QMainWindow) -> None:
    match hasattr(main_window, "profile_submenu"):
        ...
        case True:
            ...
            for profile_name in call_all_profiles():
                action = QAction("Aplicar " + build_profile_label(profile_name), main_window)
...
def process_all_settings_apply(main_window: QMainWindow) -> None:
    ...
    process_profile_save(main_window.all_widgets, main_window.current_profile)
    process_preview_start(main_window)
    process_notification_display(main_window, "Perfil '" + main_window.current_profile + "' salvo. Inicie o jogo novamente para aplicar.")
    return None
